...
namespace IL2CPP_Auto {
    std::unordered_map<std::string, uintptr_t> CAutoIL2CPP::m_CachedAddresses;
    HMODULE CAutoIL2CPP::m_IL2CPPModule = nullptr;

    bool CAutoIL2CPP::FindIL2CPPModule() {
        m_IL2CPPModule = GetModuleHandleA("GameAssembly.dll");
        if (!m_IL2CPPModule) {
            m_IL2CPPModule = GetModuleHandleA("UnityPlayer.dll");
        }
        return m_IL2CPPModule != nullptr;
    }

    void CAutoIL2CPP::CacheAllFunctions() {
        ...
        for (const char* funcName : functionNames) {
            uintptr_t address = (uintptr_t)GetProcAddress(m_IL2CPPModule, funcName);
            if (address) {
                m_CachedAddresses[funcName] = address;
            }
        }
    }

    bool CAutoIL2CPP::Initialize() {
        if (!FindIL2CPPModule()) {
            return false;
        }

        CacheAllFunctions();
        return !m_CachedAddresses.empty();
    }
}